...
def _tree(data, map_func, reduce_func):
    ...
    '''
    _logger.info('creating tree from {0} rows of data'.format(len(list(data))))
    entries = list(map(map_func, data))
    if len(entries) < 1:
        return {}
    out = {'children':{}}
...
def _create_key_generator(hierarchy):
    def _map_line(x):
        key = list(map(lambda y: x[y], hierarchy))
        value = {
            'amount': x['amount'],
            'revenue': x['revenue'],
            'expense': x['expense']
...
def _group(config, budget):
    ...
    if len(unused) > 0:
        _logger.warning('there are {0} unused budget items'.format(len(unused)))
...
def _prep_for_treemap(data):
    ...
    if 'children' in data:
        ...
        del data['children']
        data['values'] = list(map(_prep_for_treemap, data['values']))
    return data
